...
class UnacceptedLoansCalls extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			...
			array('customer_name, phone_number, state, date_offer_was_sent, amount, date_loan_was_requested, interest', 'length', 'max'=>45),
			array('association, sub_aggregators, agent_comment', 'length', 'max'=>200),
			array('beneficiaries_institution, comment', 'length', 'max'=>100),
			array('date_received', 'length', 'max'=>50),
			array('created_by', 'length', 'max'=>60),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, customer_name, phone_number, association, state, date_offer_was_sent, amount, sub_aggregators, date_loan_was_requested, beneficiaries_institution, date_received, comment, interest, agent_comment, created_on, created_by', 'safe', 'on'=>'search'),
		);
	}
